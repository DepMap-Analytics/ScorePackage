EssNonEss_plot<-function(DataMatrix,essgenes,nonessgenes,filename){
  if(is.list(DataMatrix)){
    ...
    RESBun<-PrRc_Curve(DataVector,EssNess$ess,EssNess$noness,display = FALSE,FDRth = 0.05)
    pdf(filename)
...
combatadj<-function(Data1,Data2,fdr=0.05,distmetric="Cor",subsetByQC=NULL,geneset=NULL,customBatch=NULL,extraBatch=NULL,std=FALSE,ess=NULL,noness=NULL,stdglobal=FALSE,postStd=FALSE,weights=NULL,qualityTH=Inf,QC=NULL,bycellline=FALSE,refbatch=NULL){
  ...
  if(!is.null(customBatch)){
    corrected<-ComBat(as.matrix(stdData),batch = customBatch,ref.batch=refbatch)
  }else{
  if(!is.null(extraBatch)){
    #allbatch<-cbind(site[colnames(combinedData)],extraBatch)
    corrected<-ComBat(as.matrix(stdData),mod = extraBatch[colnames(combinedData)],batch=site[colnames(combinedData)],ref.batch=refbatch)
  }else{
    corrected<-ComBat(as.matrix(stdData),batch = site[colnames(combinedData)],ref.batch=refbatch)
  }
  if(postStd){
    if(!is.null(ess)){
      ess_in<-intersect(ingenes,ess)
...
binarysignif<-function(corrected,ess=BAGEL_essential,noness=BAGEL_nonEssential,fdr=0.05){
  ...
  padjmat<-apply(pvalmat,2,p.adjust)
...
tsneplot_cp<-function(combatoutput,sitedata,filename,colourgroups,weights=NULL,COLS=NULL,perplexity=30,init_dims=30,nit=1000){
  ...
  pdf(paste0(filename,"_combatTSNE.pdf"),useDingbats = FALSE)
...
splineadjustment<-function(data,quantiles=c(0.25,0.5,0.75)){
  ...
  knots <- quantile(GeneMeans, p = quantiles)
...
compareADaMoutput<-function(dir1,dir2,filesuffix){
  ...
  for(i in 1:length(compareType)){
    ...
    fdrBH1<-p.adjust(res1[,"FEATURE_ANOVA_pval"])
    fdrBH2<-p.adjust(res2[,"FEATURE_ANOVA_pval"])
...
Allanova<-function (MoBEM, dataset) {
...
FindPower<-function(inputset,integratedset){
  ...
  criteria<-list()
    for(i in 1:length(tissues)){
      ...
      i1<-i1[bothtest,]
      i2<-i2[bothtest,]
      i1$fdr<-p.adjust(i1$p)
      i2$fdr<-p.adjust(i2$p)
      f1<-i1[sign(i1$delta)==sign(i2$delta),]
      f2<-i2[sign(i1$delta)==sign(i2$delta),]
      signInt<-f2[f2$fdr<0.05&f1$fdr>0.05,]
      signInt$nposInd<-f1[f2$fdr<0.05&f1$fdr>0.05,"npos"]
      signInt$nnegInd<-f1[f2$fdr<0.05&f1$fdr>0.05,"nneg"]
      signInt$delta<-f1[f2$fdr<0.05&f1$fdr>0.05,"delta"]
      if(nrow(signInt)>0){
...
FindSignif<-function(inputset,integratedset){
  tissues<-intersect(names(inputset),names(integratedset))
  corTest<-list()
  criteria<-list()
  for(i in 1:length(tissues)){
    ...
    signInt<-i2[i2$fdr<0.05&i1$fdr>0.05,]
    signInt$nposInd<-i1[i2$fdr<0.05&i1$fdr>0.05,"npos"]
    signInt$nnegInd<-i1[i2$fdr<0.05&i1$fdr>0.05,"nneg"]
    signInt$delta<-i1[i2$fdr<0.05&i1$fdr>0.05,"delta"]
      if(nrow(signInt)>0){
        signInt$tissue<-tissues[i]}
    criteria[[i]]<-signInt
    }
...
plotAssociationExamplesCP<-function(IndividData,IntegratedData,sel,MoBEM,cmp,AnovaData,pancan=FALSE)
{
  ...
  beeswarm(essB ~ pattern2, corral = "wrap", bg = c(makeTransparent("gray"),
           ...
           labels = c("absent", "present"), xlab = feat, ylab = "",axes=F)
  par(new = TRUE)
  boxplot(essB ~ pattern2, col = NA, ylim = range(essB), frame.plot = FALSE,
          xaxt = "n", outline = FALSE,tcl=0.5,tck=-0.01)
}

ADAMwrapper<-function(BinaryMat){
  pprofile<-ADAM2.panessprofile(depMat=BinaryMat)
  ...
  nullmodel<-ADAM2.generateNullModel(depMat=BinaryMat,ntrials = 1000)
  ...
  coreFitnessGenes<-rownames(BinaryMat)[rowSums(BinaryMat)>=crossoverpoint]
...
PowerPlot<-function(inputset,integratedset){
  tissues<-intersect(names(inputset),names(integratedset))
  values<-list()
  valuenames<-c()
  j=1
  for(i in 1:length(tissues)){
    ...
    i1$fdr<-p.adjust(i1$p)
    i2$fdr<-p.adjust(i2$p)
    sel<-(which(i2$fdr<0.05&(i2$fdr<i1$fdr)))
    ii1<-i1[sel,]
    ii2<-i2[sel,]
...
FindNewSignif<-function(inputset,integratedset){
  tissues<-intersect(names(inputset),names(integratedset))
  corTest<-list()
  criteria<-list()
  for(i in 1:length(tissues)){
    ...
    signInt<-intNew[intNew$fdr<0.05,]
...
combatadj<-function(Data1,Data2,fdr=0.05,normQ=FALSE,distmetric="Cor",subsetByQC=NULL,geneset=NULL,customBatch=NULL,extraBatch=NULL,std=FALSE,ess=NULL,noness=NULL,stdglobal=FALSE,postStd=FALSE,weights=NULL,qualityTH=Inf,QC=NULL,bycellline=FALSE,refbatch=NULL){
  ingenes<-intersect(rownames(Data1),rownames(Data2))
  combinedData<-cbind(Data1[ingenes,],Data2[ingenes,])
  if(!is.null(subsetByQC)){
    clnames<-str_split(colnames(combinedData),'---')
...
BatchCorrection<-function(data1,data2,site1="Broad",site2="Sanger",normQ=FALSE,CombatRes,stdPrior=TRUE,qcThresh=NULL,qcvalues1=NULL,qcvalues2=NULL){
  #need to make sure it's broad first then sanger for passing to adjustnewdata
  if(!is.null(qcThresh)){
    data1<-data1[,qcvalues1>=qcThresh]
    data2<-data2[,qcvalues2>=qcThresh]
  ...
  site=c(rep(site1,ncol(data1)),rep(site2,ncol(data2)))
  adjusted<-AdjustNewData(data1,data2,CombatRes,site,stdPrior,normQ)
  return(adjusted)
}

AdjustNewData<-function(data1,data2,CombatRes,site,stdPrior=TRUE,normQ=FALSE){
  blevels<-levels(as.factor(site))
  batch.design2<-CombatRes$batchDesign
  mean.star<-CombatRes$gamma.star
  var.star<-CombatRes$delta.star
...
ExtractPCs<-function(data,pcnos=1){
  pcaOall<-prcomp(t(data),scale.=TRUE)

  correctedData<-sapply(pcnos,function(x) t(pcaOall$rotation[,x])%*%data)
  return(correctedData)
}
RemovePC<-function(data,droppcanumber=1,perfCheck=TRUE){
  if(sum(is.na(data))!=0){
